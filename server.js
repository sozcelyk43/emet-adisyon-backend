    ...
    // --- Sunucu Verileri ---
    let users = [
        { id: 1, username: 'hamza', password: 'ham.za', role: 'cashier' }, // Kasa - Sadece Hızlı Satış
        { id: 2, username: 'bilal', password: 'bil.al', role: 'cashier' }, // Kasa - Tam Yetki
        { id: 10, username: 'sinan', password: 'sinan12', role: 'cashier' },// Kasa - Tam Yetki
    ...
    async function logActivity(username, actionType, details = {}, targetEntity = null, targetEntityId = null, ipAddress = null) {
        // activity_log tablosu yoksa loglama yapma veya hatayı yakala
        try {
        ...
        ws.on('message', async (messageAsString) => {
            ...
            switch (type) {
                ...
                case 'get_users':
                    ...
                    if (currentUserInfo.username === 'hamza') { // "hamza" kullanıcısı bu işlemi yapamaz
                ...
                case 'add_order_item':
                    ...
                    if (currentUserInfo.username === 'hamza') { // "hamza" kullanıcısı masaya sipariş ekleyemez
                        ws.send(JSON.stringify({ type: 'error', payload: { message: 'Masalara sipariş ekleme yetkiniz yok (hamza).' } }));
                        return;
                    }
                ...
                case 'add_manual_order_item':
                     ...
                     if (currentUserInfo.username === 'hamza') {
                        ws.send(JSON.stringify({ type: 'error', payload: { message: 'Bu işlem için yetkiniz yok (hamza).' } }));
                        return;
                    }
